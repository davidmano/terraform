...
locals {
  ...
  az = {
    ...
    network_interface = {
      name        = substr(join("-", compact([local.prefix, "-ni"])), 0, 80)
      name_unique = substr(join("-", compact([local.prefix, "-ni"])), 0, 80)
      dashes      = true
      slug        = "nic"
      min_length  = 1
      max_length  = 80
    ...
    resource_group = {
      name        = substr(join("-", compact([local.prefix, "rg", local.suffix, "frc"])), 0, 90)
